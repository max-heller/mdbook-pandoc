...
impl<'book> Preprocessor<'book> {
    pub fn new(ctx: RenderContext<'book>, markdown: &'book MarkdownConfig) -> anyhow::Result<Self> {
        ...
        copy_recursive(ctx.book.destination.as_path(), &ctx.book.source_dir, &preprocessed, &ignore)?;
...
fn copy_recursive(dest_dir: &Path, src: &Path, dst: &Path, ignore: &Gitignore) -> anyhow::Result<()> {
    for e in src
        .read_dir()
