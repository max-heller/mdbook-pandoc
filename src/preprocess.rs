...
impl<'book> Iterator for PreprocessChapter<'book, '_> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        let parse_html = |this: &mut Self, mut html: CowStr<'book>| {
            ...
            if html.contains("Renaming") {
                dbg!(&html);
            }
            let mut pandoc = Command::new("pandoc")
                .args(["-f", "html", "-t", &format])
                .stdin(Stdio::piped())
                .stdout(Stdio::piped())
                .stderr(Stdio::piped())
                .spawn()
            ...
            if output.status.success() {
                ...
                html = String::from_utf8(output.stdout).unwrap().into();
            } else {
                log::warn!(
            ...
            if let OutputFormat::Latex { packages } = &mut this.preprocessor.ctx.output {
                    ...
                    .is_available()
                {
                    ...
                    html = match FONT_AWESOME_ICON_SPAN
